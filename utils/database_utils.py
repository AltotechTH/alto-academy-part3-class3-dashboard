...
config = file_utils.load_yaml('config.yaml')
cratedb_settings = config.get('CRATEDB', dict())
...
def _execute_query_string(query_string: str):
    """
    ...
    res = list()
    try:
        cratedb_url = str(cratedb_settings['host']) + ':' + str(cratedb_settings['port'])
        connection = client.connect(cratedb_url, username=cratedb_settings.get('username'),
                                    password=cratedb_settings.get('password'))
...
def _convert_timestamp_column_to_datetime_index(df: pd.DataFrame, timestamp_column: str = 'timestamp',
    ...
    """
    df[timestamp_column] = pd.to_datetime(df[timestamp_column], unit=timestamp_unit,
                                          utc=True)  # 2022-08-28 17:37:03+00:00
...
def query_data_from_database(filters: dict, **kwargs):
    """
    ...
    logging.debug(f"Querying data: {query_string}")
    data: list = _execute_query_string(query_string)
    logging.debug(f"Finished querying data")
    if not data:
        logging.debug(f"No data found for query: {query_string}")
        return pd.DataFrame()
